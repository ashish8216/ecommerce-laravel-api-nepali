...
$data =[
    ...
    'txAmt'=> 5,
    'tAmt'=> 100,
    'pid'=>'ee2c3ca1-696b-4cc5-a6be-2c40d92jfdkjf9d453',
    'scd'=> 'epay_payment',
...
//       if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            Session::put('cart_ids',$request->cart_id);
            foreach ($carts as $key => $cartItem) {
                $tax += $cartItem['tax'] * $cartItem['quantity'];
                $subtotal += $cartItem['price'] * $cartItem['quantity'];
                
                if ($request['shipping_type_' . $cartItem['owner_id']] == 'pickup_point') {
                    $cartItem['shipping_type'] = 'pickup_point';
                    $cartItem['pickup_point'] = $request['pickup_point_id_' . $cartItem['owner_id']];
                } else {
...
//            if(Auth::user()->club_point->points >= $point) {
    ...
    public function order_confirmed()
    {
        ...
        Cart::whereIn('id', Session::get('cart_ids'))
                ->where('user_id', $order->user_id)
                ->delete();
