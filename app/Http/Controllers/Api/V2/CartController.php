...
{
    public function summary($user_id, $owner_id,$cart_id='')
    {
        
        if($cart_id && $owner_id){
             $items = Cart::where('user_id', $user_id)->where('id',$cart_id)->where('owner_id', $owner_id)->get();
             ...
             $items = Cart::where('user_id', $user_id)->where('slelected_cart',1)->get();  
             }
        ...
        $sum += get_setting('flat_rate_shipping_cost');
        ...
        return response()->json([
            ...
            'shipping_cost' => format_price(get_setting('flat_rate_shipping_cost')),
            ...
            foreach ($owner_ids as $owner_id) {
                ...
                if (!empty($shop_items_raw_data)) {
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        ...
                        $shop_items_data_item["selected"] = $shop_items_raw_data_item["selected"];
                    ...
                    //   if(!empty($shop_items_raw_data_item['variation'])){
                    //       $shop_items_data_item["upper_limit"] = @$product->stocks->where('variant', @$shop_items_raw_data_item['variation'])->first()->qty;
                    //   }else{
                    //       $shop_items_data_item["upper_limit"] ='';
                    //   }
...
//    }
    ...
    public function process(Request $request)
    {
        ...
        if($request->value){
          $slelected_cart = explode(",", $request->value);  
        }
        ...
        if (!empty($cart_ids)) {
            ...
            foreach ($cart_ids as $cart_id) {
                if($slelected_cart){
                  Cart::where('id', $cart_id)->update(['slelected_cart'=>$slelected_cart[$i]]);  
                }
