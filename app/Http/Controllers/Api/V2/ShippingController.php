...
class ShippingController extends Controller
{
    public function shipping_cost(Request $request)
    {
        ...
        if($request->cart_id){
            $shop_items_raw_data = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->where('id',$request->cart_id)->get();
        }elseif($request->owner_id){
            $shop_items_raw_data = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->get();
            }else{
             $shop_items_raw_data = Cart::where('user_id', $request->user_id)->where('slelected_cart', 1)->get();
        }
        ...
        //Total shipping cost $calculate_shipping
         ...
         if($request->cart_id){
              $total_shipping_cost = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->where('id',$request->cart_id)->sum('shipping_cost');
           
        }else{
              $total_shipping_cost = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->where('id',$request->cart_id)->sum('shipping_cost');
        }
        $total_shipping_cost = get_setting('flat_rate_shipping_cost'); 
