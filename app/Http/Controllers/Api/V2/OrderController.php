...
class OrderController extends Controller
{
    ...
    {
        if($request->cart_id){
            $cartItems = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->where('id',$request->cart_id)->get();
        
        }elseif($request->owner_id){
             $cartItems = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->get();
            }else{
            $cartItems = Cart::where('user_id', $request->user_id)->where('slelected_cart',1)->get();
        }
        ...
        if ($cartItems->first()->coupon_code != '') {
           ...
           if($request->cart_id){
                 Cart::where('user_id', $request->user_id)->where('id',$request->cart_id)->where('owner_id', $request->owner_id)->delete();
           }elseif($request->owner_id){
                       Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->delete();
          
               }else{
               Cart::where('user_id', $request->user_id)->where('slelected_cart', 1)->delete();
           }
