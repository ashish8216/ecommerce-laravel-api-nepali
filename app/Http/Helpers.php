...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        elseif (OtpConfiguration::where('type', 'twillo')->first()->value == 1) {
         ...
         elseif (OtpConfiguration::where('type', 'akash')->first()->value == 1) {
            ...
            'auth_token'=> '6a9b20ce8f32fbfa15bdd739293a64c3e423595acff3567e7aa6cd0384891e70',
            'to'    => $to,
            'text'  => $text));
...
if (!function_exists('get_images')) {
    ...
    {
        if(is_null($given_ids)){
            $ids = [];
        }elseif(is_array($given_ids)){
            $ids = $given_ids;
        }else{
