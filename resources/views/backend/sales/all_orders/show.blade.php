...
<div class="card">
    ...
    <div class="card-body">
        <div class="row gutters-5">
            ...
            $childOrder = $order->childOrder;
            $totalPrice = 0;
            $totalTax = 0;
            $totalShipping = get_setting('flat_rate_shipping_cost');
            @endphp
            ...
            <div class="col-md-3 ml-auto">
                <label for="update_delivery_status">{{translate('Order Identifier Code')}}</label>
                @foreach($childOrder as $k=>$v)
                <input type="text" class="form-control mb-2" disabled='' value="{{$v->code}}">
                @endforeach
            </div>
            @endif
            ...
            @if (\App\Addon::where('unique_identifier', 'delivery_boy')->first() != null && 
                ...
                <div class="col-md-3 ml-auto">
                    ...
                    @if($childOrder->isEmpty())
                        ...
                        <select class="form-control aiz-selectpicker" data-live-search="true" data-minimum-results-for-search="Infinity" id="assign_deliver_boy" data-child="{{$order->id}}">
                    ...
                    @foreach($childOrder as $k=>$v)
                        @if($delivery_status == 'pending' || $delivery_status == 'picked_up')
                        <select class="form-control aiz-selectpicker mb-2" data-live-search="true" data-minimum-results-for-search="Infinity" id="assign_deliver_boy" data-child="{{$v->id}}">
                            <option value="">{{translate('Select Delivery Boy')}}</option>
                            @foreach($delivery_boys as $delivery_boy)
                            <option value="{{ $delivery_boy->id }}" @if($v->assign_delivery_boy == $delivery_boy->id) selected @endif>
                                {{ @$delivery_boy->name }}
                            </option>
                            @endforeach
                        </select>
                        @else
                        <input type="text" class="form-control" value="{{ @$v->delivery_boy->name }}">
                        @endif
                    @endforeach
                    @endif
                </div>
            ...
            <div class="col-md-3 ml-auto">
                ...
                @if($childOrder->isEmpty())
                    <select class="form-control aiz-selectpicker"  data-minimum-results-for-search="Infinity" id="update_payment_status" data-child="{{$order->id}}">
                        <option value="paid" @if ($payment_status == 'paid') selected @endif>{{translate('Paid')}}</option>
                        <option value="unpaid" @if ($payment_status == 'unpaid') selected @endif>{{translate('Unpaid')}}</option>
                    </select>
                @else
                    @foreach($childOrder as $k=>$v)
                        <select class="form-control aiz-selectpicker mb-2"  data-minimum-results-for-search="Infinity" id="update_payment_status" data-child="{{$v->id}}">
                            <option value="paid" @if ($payment_status == 'paid') selected @endif>{{translate('Paid')}}</option>
                            <option value="unpaid" @if ($payment_status == 'unpaid') selected @endif>{{translate('Unpaid')}}</option>
                        </select>
                    @endforeach
            ...
            @if($childOrder->isEmpty())
                ...
                <div class="col-md-3 ml-auto">
                    ...
                    @if($delivery_status != 'delivered' && $delivery_status != 'cancelled')
                        <select class="form-control aiz-selectpicker "  data-minimum-results-for-search="Infinity" id="update_delivery_status" data-child="{{$order->id}}">
            ...
            @else
                @php $status = @$childOrder[0]->orderDetails[0]->delivery_status; @endphp
           
                <div class="col-md-3 ml-auto">
                    <label for="update_delivery_status">{{translate('Delivery Status')}}</label>
                    @foreach($childOrder as $k=>$v)
                        <select class="form-control aiz-selectpicker mb-2"  data-minimum-results-for-search="Infinity" id="update_delivery_status" data-child="{{$v->id}}">
                            <option value="pending" @if ($delivery_status == 'pending') selected @endif>{{translate('Pending')}}</option>
                            <option value="confirmed" @if ($delivery_status == 'confirmed') selected @endif>{{translate('Confirmed')}}</option>
                            <option value="picked_up" @if ($delivery_status == 'picked_up') selected @endif>{{translate('Picked Up')}}</option>
                            <option value="on_the_way" @if ($delivery_status == 'on_the_way') selected @endif>{{translate('On The Way')}}</option>
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            ...
                            @php
                            //Change order delivery status on the order loop above OrderNote
                ...
                <table class="table table-bordered aiz-table invoice-summary">
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach($childOrder as $k=>$v)
                            @php 
                                $totalPrice += $v->orderDetails->sum('price');
                                $totalTax += $v->orderDetails->sum('tax');
                                // $totalShipping += $v->orderDetails->sum('shipping_cost');
                            @endphp
                            <tr><td></td><td></td><td>Package #{{$k+1}}</td></tr>
                                @foreach($v->orderDetails as $key=>$orderDetail)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>
                                            @if ($orderDetail->product != null)
...
@section('script')
    <script type="text/javascript">
        $('#assign_deliver_boy').on('change', function(){
            ...
            var order_id = $(this).attr('data-child');
